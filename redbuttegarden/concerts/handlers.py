...
def concert_page_changed(concert_page):
    logger.debug(
        "Generating new iCal file with concerts from %s...", concert_page.title
    )
    c = Calendar()
    ...
    for concert in concerts:
        for concert_date in concert["concert_dates"]:
            ...
            if concert["show_time"]:
                ...
                e.begin = event_start
                e.end = event_start + datetime.timedelta(
                    hours=3
                )  # Assuming concerts last 3 hours
            else:
    ...
    with default_storage.open(
        f"concert_calendar_{concert_page.slug}.ics", mode="w"
    ) as cal_file:
        if (
            settings.STORAGES["default"]["BACKEND"]
            == "home.custom_storages.MediaStorage"
        ):
    ...
    logger.debug(
        "iCal file generated successfully: %s",
        f"concert_calendar_{concert_page.slug}.ics",
    )

    batch = PurgeBatch()
    batch.add_url(f"/media/concert_calendar_{concert_page.slug}.ics")
    batch.purge()
