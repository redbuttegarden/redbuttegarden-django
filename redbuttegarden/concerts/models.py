...
class Sponsors(blocks.StructBlock):
    ...
    sponsor_url = blocks.URLBlock(label="URL to sponsor website")
    sponsor_logo = ImageBlock()


class SponsorList(blocks.StructBlock):
    list_items = blocks.ListBlock(Sponsors(), label="Sponsors")
    ...
    table_list = blocks.StreamBlock(
        [
            ...
            (
                "table",
                TableBlock(
                    table_options=donor_table_options,
...
class TableInfoCardList(blocks.StructBlock):
    list_items = blocks.ListBlock(
        TableInfoCard(), label="List of info cards with tables"
    )
...
class ConcertBlock(blocks.StructBlock):
    ...
    wave = blocks.ChoiceBlock(
        choices=[(0, "Presale"), (1, "Wave 1"), (2, "Wave 2")], required=False
    )
    hidden = blocks.BooleanBlock(
        default=True,
        help_text=_(
            "If hidden box is checked, concert will not be displayed on" " the page"
        ),
        required=False,
    )
    ...
    virtual = blocks.BooleanBlock(
        default=False, help_text=_("Is this a virtual concert?"), required=False
    )
    ...
    band_name = blocks.CharBlock(
        ...
        max_length=200,
        help_text=_(
            "Optional but recommended name of the band(s)/artist(s) performing. Text in this field is not displayed on the concert page so you should also include the band names in the Band Info section below. This field is used when generating iCal event titles and for displaying concert buy links on the home page on the day of the concert if not sold out."
        ),
        required=False,
        blank=True,
    )
    ...
    gates_time = blocks.TimeBlock(
        default=datetime.time(hour=18), required=False, blank=True, null=True
    )
    show_time = blocks.TimeBlock(
        default=datetime.time(hour=19), required=False, blank=True, null=True
    )
...
class ConcertPage(AbstractBase):
        ...
        if self.wave_two_sale_date and datetime.date.today() < self.wave_two_sale_date:
            context["presale_concerts"] = self.sort_concerts(
                [
                    ...
                    if concert["wave"] and concert["wave"] == "0"
                ]
            )
            context["wave_one_concerts"] = self.sort_concerts(
                [
                    concert
                    for concert in concerts
                    if concert["wave"] and concert["wave"] == "1"
                ]
            )
            context["wave_two_concerts"] = self.sort_concerts(
                [
                    concert
                    for concert in concerts
                    if concert["wave"] and concert["wave"] == "2"
                ]
            )
        else:
    ...
    def get_visible_concerts(self):
        return [
            concert.value
            for concert in self.body
            if concert.block_type == "concerts"
            and not concert.value["hidden"]
            and len(concert.value["concert_dates"]) > 0
        ]
    ...
    body = StreamField(
        block_types=[
            ...
            (
                "table",
                TableBlock(
                    table_options=donor_schedule_table_options,
                    help_text=_("Right-click to add/remove rows/columns"),
                ),
            ),
        ],
        ...
        if request.user.is_authenticated:
            ...
            if cdc_member:
                ...
                messages.add_message(
                    request,
                    messages.WARNING,
                    "No matching Concert Donor Club membership found.",
                )
...
class ConcertDonorClubTicketSalePage(AbstractBase):
    ...
    def get_context(self, request, *args, **kwargs):
        ...
        chat_user_response = requests.get(
            f"https://api.deadsimplechat.com/consumer/api/v2/user/{request.user.id}",
        ...
        if chat_user_response.status_code == 400:
            ...
            payload = {
                "membershipDetails": {
                    ...
                    ),  # Use admin role for staff users
            ...
            chat_user_creation_response = requests.post(
...
class ConcertDonorClubPackage(models.Model):
    ...
    year = models.IntegerField(
        _("year"), validators=[MinValueValidator(1984), MaxValueValidator(2099)]
    )
...
class ConcertDonorClubMember(models.Model):
    ...
    chat_access_token = models.CharField(
        ...
        null=True,
        help_text=_(
            "This is used to store the Dead Simple Chat access token for this user."
        ),
    )
...
class Ticket(models.Model):
    ...
    package = models.ForeignKey(
        ConcertDonorClubPackage, on_delete=models.SET_NULL, null=True, blank=True
    )
