    ...
    try:
        if scientific_name:
            collections = collections.filter(
                species__full_name__icontains=scientific_name
            )
        if common_name:
            collections = collections.annotate(
                search=SearchVector("species__cultivar", "species__vernacular_name")
            ).filter(search=common_name)
        if family:
        ...
        if flower_color:
            collections = collections.filter(
                species__flower_color__icontains=flower_color
            )
        if memorial_person:
