...
class CollectionSerializer(serializers.ModelSerializer):
    ...
    plant_date = serializers.DateField(
    ...
    class Meta:
        ...
        extra_kwargs = {"plant_id": {"validators": []}}
...
class SpeciesImageListSerializer(serializers.ModelSerializer):
    species_full_name = serializers.CharField(
        source="species.full_name", read_only=True
    )
    image_id = serializers.IntegerField(source="image.id", read_only=True)
    image_url = serializers.SerializerMethodField()
    image_description = serializers.SerializerMethodField()
    ...
    def get_image_url(self, obj):
        f = getattr(obj.image, "file", None)
        return f.url if f else None

    def get_image_description(self, obj):
        # Only if your Image model actually has a 'description' field
        return getattr(obj.image, "description", None)


class SpeciesImageEditImageDescriptionSerializer(serializers.Serializer):
    species_full_name = serializers.CharField(
        source="species.full_name", read_only=True
    )
    image_id = serializers.IntegerField(source="image.id", read_only=True)
    ...
    def update(self, instance: SpeciesImage, validated_data):
        img = instance.image
        if not hasattr(img, "description"):
            raise serializers.ValidationError(
                {"description_write": "Image has no 'description' field."}
            )

        new_desc = validated_data["description_write"]
        img.description = new_desc
        img.save(update_fields=["description"])
        return instance
